// This script mimics the deploy_vps_runner.js but only runs a database query to check sellers and leads
const { Client } = require('ssh2');
const conn = new Client();

console.log('Connecting to VPS to run database query...');

conn
  .on('ready', () => {
    console.log('SSH connected');
    // Change to the project directory and run a node script that checks the database
    const command = `
      set -e
